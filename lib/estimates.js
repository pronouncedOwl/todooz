...
const ALLOWED = new Set(ESTIMATE_OPTIONS.map((o) => o.minutes));

/** Normalize raw input to a allowed preset or null. */
export function normalizeEstimateMinutes(raw) {
  if (raw === null || raw === undefined || raw === "") return null;
  const n = Number(raw);
  if (!Number.isInteger(n) || !ALLOWED.has(n)) {
    throw new Error("estimate_minutes must be one of: 15, 30, 60, 120, 240");
  }
  return n;
}
