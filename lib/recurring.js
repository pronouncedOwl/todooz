...
import { HABIT_CLOSEOUT_BLOCKS, todayISO } from "@/lib/dates";
...
export const TIME_OF_DAY = new Set([
  "before_wake",
  "morning",
  "daytime",
  "evening",
...
function emptyBlockCloseouts() {
  return Object.fromEntries(HABIT_CLOSEOUT_BLOCKS.map((block) => [block.id, null]));
}

function normalizeBlockCloseouts(raw, morningClosedOn = null) {
  const next = emptyBlockCloseouts();
  ...
  if (
    !next.morning &&
    typeof morningClosedOn === "string" &&
    /^\d{4}-\d{2}-\d{2}$/.test(morningClosedOn)
  ) {
    next.morning = morningClosedOn;
  }
  return next;
}

function normalizeHabitPrefs(prefs) {
  const block_closeouts = normalizeBlockCloseouts(
    prefs?.block_closeouts,
    prefs?.morning_closed_on,
  );
  return {
    morning_closed_on: block_closeouts.morning,
    block_closeouts,
  };
}
...
async function readLocalStore() {
  ...
  return {
    ...
    prefs: normalizeHabitPrefs(data.prefs),
  };
}
...
async function readRemoteStore(userId, supabase) {
  const [templatesRes, instancesRes] = await Promise.all([
    supabase
      ...
      .select(
        "id, title, freq, interval, times_per_day, byweekday, anchor, notes, estimate_minutes, time_of_day, time_slots, created_at",
      )
      .eq("user_id", userId),
...
export async function getHabitPrefs() {
  if (supabaseEnabled()) {
    ...
    const { data, error } = await supabase
      .from("habit_prefs")
      .select("morning_closed_on, block_closeouts")
      .eq("user_id", user.id)
      .maybeSingle();
    if (error) throw new Error(error.message);
    return normalizeHabitPrefs(data);
  }

  const store = await readLocalStore();
  return normalizeHabitPrefs(store.prefs);
}

export async function setBlockClosedOn(blockId, dateISO) {
  if (!HABIT_CLOSEOUT_BLOCKS.some((block) => block.id === blockId)) {
    throw new Error(`Unknown habit block: ${blockId}`);
  }
  const day =
  ...
  if (supabaseEnabled()) {
    ...
    const { data, error: fetchError } = await supabase
      ...
      .maybeSingle();
    if (fetchError) throw new Error(fetchError.message);
    const next = normalizeHabitPrefs(data);
    next.block_closeouts[blockId] = day;
    next.morning_closed_on = next.block_closeouts.morning;
    const { error } = await supabase.from("habit_prefs").upsert(
      {
        user_id: user.id,
        morning_closed_on: next.morning_closed_on,
        block_closeouts: next.block_closeouts,
      },
      { onConflict: "user_id" },
    );
    if (error) throw new Error(error.message);
  ...
  return withStoreLock(async () => {
    const store = await readLocalStore();
    const next = normalizeHabitPrefs(store.prefs);
    next.block_closeouts[blockId] = day;
    next.morning_closed_on = next.block_closeouts.morning;
    store.prefs = next;
    await writeLocalStore(store);
...
const CLOSEOUT_ACTIONS = new Set(["complete", "skip", "leave_open"]);

/**
 * Resolve leftover open habits for a time-of-day block after its cutoff.
 * Leave-open items stay on the list; the prompt will not return until tomorrow.
 * @param {string} blockId
 * @param {Array<{ id: string, action: "complete"|"skip"|"leave_open" }>} resolutions
 */
export async function resolveHabitBlockLeftovers(blockId, resolutions) {
  if (!HABIT_CLOSEOUT_BLOCKS.some((block) => block.id === blockId)) {
    throw new Error(`Unknown habit block: ${blockId}`);
  }
  if (!Array.isArray(resolutions)) {
    throw new Error("Resolutions are required");
  }
  ...
  for (const row of resolutions) {
    ...
    if (!CLOSEOUT_ACTIONS.has(action)) {
      throw new Error(`Invalid closeout action: ${action}`);
    }
    if (action === "complete") {
      await setRecurringCompleted(id, true);
    } else if (action === "skip") {
  ...
  const prefs = await setBlockClosedOn(blockId, today);
